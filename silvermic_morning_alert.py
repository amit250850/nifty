#!/usr/bin/env python3
"""
silvermic_morning_alert.py
==========================
Pre-market MCX SILVERMIC CPR alert.
Runs at 08:55 IST → Telegram message arrives before MCX opens at 09:00.

Strategy  : AAK CPR Breakout — 1 lot SILVERMIC, NRML product
...
---------------
  Lot size      : 1 kg
  Quote         : ₹ per kg
  P&L per lot   : ₹1 per ₹1/kg move  (1 kg × ₹1/kg)
  Commission    : ₹65 flat per round-trip (Zerodha per-order model)
  NRML margin   : ~₹62,500/lot  (~25% SEBI SPAN+EM on ₹2.5L lot value)
  Slippage      : ₹10/kg per side
  Expiry cycle  : MONTHLY (last day of each month) — roll 5 days before expiry

  1 lot on ₹87K account:
    Margin ≈ ₹62,500  →  Free ≈ ₹24,500
    P&L: ₹1 per ₹1/kg move · ₹100 move → ₹100

  Backtest (Aug 2025 – Apr 2026, 9 months):
    78 trades | 79.5% win | +₹1.22L net | 5.27× payoff | 9/0 green months
    Budget: ₹62,500 margin fits ₹87K comfortably

Run schedule
------------
  Time    : 08:55 IST  Mon–Fri
  Via main.py APScheduler (wired in at 08:55 alongside GOLDPETAL + GOLDGUINEA).
  Or standalone: python silvermic_morning_alert.py
...
LOTS         = 1
PNL_PER_LOT  = 1        # ₹1 per lot per ₹1/kg price move (1 kg × ₹1/kg)
COMMISSION   = 65       # flat per round-trip
SLIPPAGE     = 10.0     # ₹/kg per side

# CPR width thresholds (% of pivot price) — matches backtest_mcx_cpr.py
NARROW_HIGH     = 0.20   # 🔴 HIGH
NARROW_MEDIUM   = 0.30   # 🟡 MEDIUM
NARROW_STANDARD = 0.45   # 🟢 STANDARD

MARGIN_PCT   = 0.25     # ~25% NRML SEBI SPAN+EM for SILVERMIC
CAPITAL      = 87_000
...
def get_front_month(kite: KiteConnect) -> tuple:
    """Return (token, tradingsymbol, expiry) for active front-month SILVERMIC."""
    ...
    if silvermic.empty:
        raise RuntimeError("No active SILVERMIC contracts found.")

    front = silvermic.iloc[0]
    dte   = (front["expiry"].date() - date.today()).days
    if dte <= 5 and len(silvermic) > 1:   # roll 5 days before expiry
        front = silvermic.iloc[1]
        dte   = (front["expiry"].date() - date.today()).days
...
def build_skip_message(symbol, expiry, prev_date, cpr, width, today_str, dow):
    return (
        f"🪙 <b>SILVERMIC CPR — {today_str} ({dow})</b>\n"
...
def build_trade_message(symbol, expiry, prev_date, cpr,
    ...
    # Risk / reward in ₹ (PNL_PER_LOT = ₹1 per ₹1/kg move)
    ...
    return (
        ...
        f"  1 kg/lot  ·  P&L ₹{PNL_PER_LOT}/lot per ₹1/kg move\n"
...
# ─── MAIN ─────────────────────────────────────────────────────────────────────
def send_silvermic_cpr_alert() -> None:
    now       = datetime.now()
    today_str = now.strftime("%a %d %b %Y")
    dow       = now.strftime("%A")

    log.info("=" * 56)
    log.info("SILVERMIC CPR Alert  —  %s  08:55 IST", today_str)
    log.info("=" * 56)
