#!/usr/bin/env python3
"""
nickel_morning_alert.py
=======================
Pre-market MCX NICKEL CPR alert.
Runs at 08:55 IST → Telegram arrives before MCX opens at 09:00.

Strategy  : AAK CPR Breakout — 1 lot NICKEL, NRML product
Conviction: based on CPR width (< 0.40% = narrow/breakout day)

NICKEL specs
------------
  Lot size      : 250 kg  (MCX reduced from 1,500 kg)
  Quote         : ₹ per kg
  P&L per lot   : ₹250 per ₹1/kg move  (250 kg × ₹1)
  Commission    : ₹65 flat per round-trip
  NRML margin   : ~₹48,866/lot  (Zerodha NRML, verified Apr 2026)
  Slippage      : ₹0.20/kg per side (2 ticks)
  Expiry cycle  : MONTHLY — roll 5 days before expiry

  1 lot on ₹87K account:
    Margin ≈ ₹48,866  →  Buffer ≈ ₹38,134
    P&L: ₹250 per ₹1/kg move · ₹10 move → ₹2,500

Backtest (Mar–Apr 2026, 1 lot):
  76.9% win rate · 3.85x payoff · +₹1.31L in 6 weeks
  NOTE: Only 13 trades — Kite hourly data limited to ~6 weeks.
        Paper trade for 4+ weeks before going live.

Run schedule
------------
  Time  : 08:55 IST  Mon–Fri
  Via main.py APScheduler. Or standalone: python nickel_morning_alert.py
"""
...
LOTS            = 1
LOT_KG          = 250        # 250 kg per lot (MCX reduced from 1,500 kg)
PNL_PER_LOT     = 250        # ₹250 per ₹1/kg move
COMMISSION      = 65
SLIPPAGE        = 0.20       # ₹/kg per side (2 ticks)
MARGIN_PER_LOT  = 48_866     # actual Zerodha NRML (verified Apr 2026)
CAPITAL         = 87_000

# CPR width thresholds (% of pivot price)
NARROW_HIGH     = 0.10   # HIGH conviction
NARROW_MEDIUM   = 0.20   # MEDIUM conviction
NARROW_STANDARD = 0.40   # STANDARD (matches backtest narrow_pct)
...
def get_front_month(kite: KiteConnect) -> tuple:
    """Return (token, tradingsymbol, expiry) for active front-month NICKEL."""
    ...
    if contracts.empty:
        raise RuntimeError("No active NICKEL contracts found.")
...
def build_skip_message(symbol, expiry, prev_date, cpr, width, today_str, dow):
    return (
        f"🔩 <b>NICKEL CPR — {today_str} ({dow})</b>\n"
...
def build_trade_message(symbol, expiry, prev_date, cpr,
    ...
    return (
        ...
        f"  {LOT_KG:,} kg/lot  ·  P&L ₹{PNL_PER_LOT:,}/lot per ₹1/kg move\n"
        f"  Margin   ~{_r(margin_tot)}   Buffer  ~{_r(buffer)}\n"
        f"  ⚠️  Margin is tight — keep ₹55K+ capital for safety\n"
        f"  Max loss  ~{_r(max_loss)}  ({loss_pct:.1f}% of capital)\n"
        f"  P&L/₹10 move  {_r(PNL_PER_LOT * LOTS * 10)}   "
        f"P&L/₹100 move  {_r(PNL_PER_LOT * LOTS * 100)}\n\n"
        f"<i>Backtest: 77% win · 3.85x payoff · 6 weeks data (paper trade first!).\n"
        f"EOD exit 17:00 IST.</i>"
    )


# ─── MAIN ─────────────────────────────────────────────────────────────────────
def send_nickel_cpr_alert() -> None:
    now       = datetime.now()
    today_str = now.strftime("%a %d %b %Y")
    dow       = now.strftime("%A")

    log.info("=" * 56)
    log.info("NICKEL CPR Alert  —  %s  08:55 IST", today_str)
    log.info("=" * 56)
