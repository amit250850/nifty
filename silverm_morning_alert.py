#!/usr/bin/env python3
"""
silverm_morning_alert.py
========================
Pre-market MCX SILVERM CPR alert.
Runs at 08:55 IST → Telegram message arrives before MCX opens at 09:00.

Strategy  : AAK CPR Breakout — 1 lot SILVERM, NRML product
...
-------------
  Lot size      : 5 kg
  Quote         : ₹ per kg
  P&L per lot   : ₹5 per ₹1/kg move  (5 kg × ₹1/kg)
  Commission    : ₹65 flat per round-trip (Zerodha per-order model)
  NRML margin   : ~₹35,000/lot  (~10% SEBI SPAN+EM on ₹3.5L lot value)
  Slippage      : ₹10/kg per side (wider spread vs gold)
  Expiry cycle  : MONTHLY (last Thursday of each month) — roll 5 days before expiry

  1 lot on ₹50K account:
    Margin ≈ ₹35,000  →  Free ≈ ₹15,000
    P&L: ₹5 per ₹1/kg move · ₹100 move → ₹500

  vs GOLDPETAL (10 lots):
    SILVERM 1L : ₹35,000 margin, ₹5 per ₹1/kg move, monthly expiry
    GOLDPETAL 10L: ₹22,000 margin, ₹10 per ₹1 move, monthly expiry

  vs GOLDGUINEA (2 lots):
    SILVERM 1L: ₹35,000 margin, monthly expiry, higher volatility
    GOLDGUINEA 2L: ₹35,500 margin, bimonthly expiry, lower volatility

Run schedule
------------
  Time    : 08:55 IST  Mon–Fri
  Via main.py APScheduler (wired in at 08:55 alongside GOLDPETAL + GOLDGUINEA).
  Or standalone: python silverm_morning_alert.py
...
LOTS         = 1
PNL_PER_LOT  = 5        # ₹5 per lot per ₹1/kg price move (5 kg × ₹1/kg)
COMMISSION   = 65       # flat per round-trip
SLIPPAGE     = 10.0     # ₹/kg per side

# CPR width thresholds (% of pivot price)
# Silver is more volatile than gold → thresholds are slightly wider
NARROW_HIGH     = 0.20   # 🔴 HIGH
NARROW_MEDIUM   = 0.30   # 🟡 MEDIUM
NARROW_STANDARD = 0.40   # 🟢 STANDARD  (matches backtest_mcx_cpr.py SILVERM narrow_pct)

MARGIN_PCT   = 0.10     # ~10% SEBI SPAN+EM for SILVERM
CAPITAL      = 50_000
...
def get_front_month(kite: KiteConnect) -> tuple:
    """Return (token, tradingsymbol, expiry) for active front-month SILVERM."""
    ...
    if silverm.empty:
        raise RuntimeError("No active SILVERM contracts found.")

    front = silverm.iloc[0]
    dte   = (front["expiry"].date() - date.today()).days
    if dte <= 5 and len(silverm) > 1:   # roll 5 days before expiry
        front = silverm.iloc[1]
        dte   = (front["expiry"].date() - date.today()).days
...
def get_recent_daily(kite: KiteConnect, token: int, n: int = 10) -> list:
    """Last n trading days of daily OHLC."""
...
def build_skip_message(symbol, expiry, prev_date, cpr, width, today_str, dow):
    return (
        f"🥈 <b>SILVERM CPR — {today_str} ({dow})</b>\n"
        ...
        f"  Pivot      {_p(cpr['P'])}/kg\n"
        f"  Upper CPR  {_p(cpr['upper_cpr'])}/kg\n"
        f"  Lower CPR  {_p(cpr['lower_cpr'])}/kg\n"
...
def build_trade_message(symbol, expiry, prev_date, cpr,
    ...
    # Risk / reward in ₹ (PNL_PER_LOT = ₹5 per ₹1/kg move)
    ...
    return (
        ...
        f"  5 kg/lot  ·  P&L ₹{PNL_PER_LOT}/lot per ₹1/kg move\n"
...
# ─── MAIN ─────────────────────────────────────────────────────────────────────
def send_silverm_cpr_alert() -> None:
    now       = datetime.now()
    today_str = now.strftime("%a %d %b %Y")
    dow       = now.strftime("%A")

    log.info("=" * 56)
    log.info("SILVERM CPR Alert  —  %s  08:55 IST", today_str)
    log.info("=" * 56)
